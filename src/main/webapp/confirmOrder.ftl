...
<div id="wrap">
	...
	<form action="${basepath}/auth/order/pay.html" method="post" theme="simple" onsubmit="return submitOrder();">
		<div class="container">
			<div class="row">
						...
						<#else>
							<#if currentAccount()??>
								<div class="bs-callout bs-callout-danger author" style="text-align: left;font-size: 14px;margin: 2px 0px;">
									暂时还没有收货地址！<a style="text-decoration: underline;" href="${basepath}/auth/account/address">点此设置</a>
								</div>
							</#if>
						</#if>
					</#if>
