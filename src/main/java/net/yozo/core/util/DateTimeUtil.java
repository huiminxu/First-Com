...
//            String cha = day + "天" + (hour - day * 24) + "小时"  + (min - day * 24 * 60) + "分钟" + sec + "秒";
	...
	//20171111-20171212注册送会员活动
	public static Account giveVipWhenReg(Account account)  {
		Date activityBegin= null;
		try {
			activityBegin = new SimpleDateFormat("yyyyMMdd").parse("20171111");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		Date activityEnd= null;
		try {
			activityEnd = new SimpleDateFormat("yyyyMMdd").parse("20171213");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		Date regDate=new Date();
		if(regDate.after(activityBegin) && regDate.before(activityEnd)){
			account.setVipDate(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(org.apache.commons.lang.time.DateUtils.addDays(regDate,30)));
		}
		return account;
	}
}
