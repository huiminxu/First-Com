...
public class Main {
    ...
    // 测试当面付2.0查询订单
    public String test_trade_query(Order order) {
        // (必填) 商户订单号，通过此商户订单号查询当面付的交易状态
        //String outTradeNo = orderNo;

        // 创建查询请求builder，设置请求参数
        AlipayTradeQueryRequestBuilder builder = new AlipayTradeQueryRequestBuilder()
                .setOutTradeNo(order.getOrderNum());
        ...
        switch (result.getTradeStatus()) {
            ...
            case FAILED:
                if ("WAIT_BUYER_PAY".equals(response.getTradeStatus())) {
                    ...
                    tradeStatus = "failed";
                    log.error("----zfb未付款交易超时关闭，或支付完成后全额退款");
                } else {
                    //System.out.println(response.getTradeStatus());
                    tradeStatus = "noScan";//用户尚未扫描，订单无法查询到
                    log.error("----zfb用户尚未扫描");
                }
                break;

            case UNKNOWN:
                log.error("----zfb系统异常，订单支付状态未知!!!");
                tradeStatus = "unknown";
                break;

            default:
                log.error("----zfb不支持的交易状态，交易返回异常!!!");
                tradeStatus = "error";
                break;
        }
    ...
    // 测试当面付2.0生成支付二维码
    public String test_trade_precreate( Order order) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = order.getOrderNum();

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = order.getTemplateName();

        // (必填) 订单总金额，单位为元，不能超过1亿元
        // 如果同时传入了【打折金额】,【不可打折金额】,【订单总金额】三者,则必须满足如下条件:【订单总金额】=【打折金额】+【不可打折金额】
        String totalAmount = order.getAmount();
        ...
        String body = "购买商品" + order.getTemplateName();

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "test_operator_id";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = "2088721225457885";
        ...
        // 商品明细列表，需填写购买商品详细信息，
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        long ll=Math.round(100*Double.valueOf(order.getAmount()));
        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        GoodsDetail goods1 = GoodsDetail.newInstance(order.getTemplateId()+"", order.getTemplateName(),ll, 1);
        // 创建好一个商品后添加至商品明细列表
        goodsDetailList.add(goods1);
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://www.yomoer.cn/paygate/zfbNotify")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                /*String filePath = String.format("C:/Users/Vicks/Desktop/qr-%s.png",
                    response.getOutTradeNo());
                log.info("filePath:" + filePath);*/
                //              ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);

                /*String basePath = "F:\\project\\ymfront\\target\\ymshop\\";
                ...
                //getQRCodeImge(resp, response.getQrCode(), 256, 256);
                zfb=getQRCodeImge2(response.getQrCode(), 256, 256);
                break;
    ...
    //二维码生成方式1，流输出
    public void getQRCodeImge(HttpServletResponse resp, String contents, int width, int height) {
        if (contents != null && !"".equals(contents)) {
            ...
            try {
                ...
                BitMatrix m = writer.encode(contents, BarcodeFormat.QR_CODE, height, width);
                try {
    ...
    public static void toAccount( ) {
        ...
        request.setBizContent("{" +
                "\"out_biz_no\":\"test31423214234332434212\"," +//订单号
                "\"payee_type\":\"ALIPAY_LOGONID\"," +//支付宝登录号，支持邮箱和手机号格式。或者ALIPAY_USERID：支付宝账号对应的支付宝唯一用户号。以2088开头的16位纯数字组成。
                "\"payee_account\":\"279884792@qq.com\"," +//与上一个参数对应的账号
                "\"amount\":\"0.1\"," +//金额、元、不得小于0.1
